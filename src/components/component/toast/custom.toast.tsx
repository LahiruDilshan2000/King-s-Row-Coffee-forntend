...
const CustomToast = (props: Props): JSX.Element => {
    return (
        <div
            ...
            <div
                className={`w-12 aspect-square ${props.title === "Success" ? 'bg-green-100' : props.title === "Error" ? 'bg-red-100' : 'bg-yellow-200'} rounded-full flex justify-center items-center mr-3`}>
            ...
            <div>
                <h1 className={`text-sm ${props.title === "Success" ? 'text-green-500' : props.title === "Error" ? 'text-red-500' : 'text-yellow-500'}`}>{props.title}</h1>
